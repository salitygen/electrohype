 ...
 * @since  3.7.0
 */
class JkFieldModel extends AdminModel
{
	/**
	 * @var null|string
	 *
	...
	public function save($data)
	{
			...
			&& isset($data['fieldparams']['options']) && isset($field->fieldparams['options']))
		{
			...
			if (is_object($oldParams) && is_object($newParams) && $oldParams != $newParams)
			{
				...
				$query = $db->getQuery(true);
				$query->delete($db->quoteName('#__jkfilter'))
					->where($db->quoteName('field_id') . ' = :fieldid')
	...
	public function getTable($name = 'Field', $prefix = 'Administrator', $options = array())
	{
		...
		$table = $factory->createTable($name, $prefix, $options);
		$table->type = 'text';
		return $table;
	}
	...
	public function delete(&$pks)
	{
		...
		if ($success)
		{
			...
			if (!empty($pks))
			{
				...
				$query->delete($query->quoteName('#__jkfilter'))
	...
	public function changeStateValues($context, $pks, $value){
		
		if(!is_array($pks) && !is_int($value)) return false;
		...
		foreach($pks as $id){
			$updateObj->item_id	= $id;
			$this->getDbo()->updateObject('#__jkfilter', $updateObj, array('item_id'));
		}
	 ...
	 * @since   3.7.0
	 * 
	 * SalitrGEN Hack!! Old Name method: setFieldValue
	 */
	public function setJkFieldValue($fieldId, $item, $value)
	{

		// Perhaps some other extensions will use the old logic
		if(is_object($item)){
			$itemId	= $item->id;
			if(isset($item->groups) && isset($item->params)){
				if(empty($lang = json_decode($item->params)->language)){
					$lang = '*';
				}
				$catId 	= $item->groups[0];
			}else{
		...
		if(isset($item->state)){
			...
			if(isset($item->block) && isset($item->activation)){
				if($item->activation == 1){
					$state = 0;
				}else{
					$state = (int)(!(boolean)$item->block);
				}
			}else{
				$state = NULL;
			}
		...
		if(isset($params->field_template) && !empty($params->field_template)){
			$field_template = $params->field_template;
		}else{
			$field_template = 'off';
		}
		
		if(isset($params->field_background) && !empty($params->field_background)){
			$field_background = $params->field_background;
		}else{
			$field_background = 'off';
		}
		
		if(isset($params->disabled_categories) && !empty($params->disabled_categories)){
		...
		if ($needsDelete){
			...
			$query->delete($query->quoteName('#__jkfilter'))
				...
				->where($query->quoteName('context') . ' = :context')
				->bind(':fieldid',$fieldId, ParameterType::INTEGER)
				->bind(':itemid',$itemId)
				->bind(':context',$field->context);
		...
		if ($needsInsert && $field->type !== 'subform'){
			...
			for($i=0;$i<$count;$i++){
				...
				$this->getDbo()->insertObject('#__jkfilter', $newObj);
			}

		}

		if($needsUpdate && $field->type !== 'subform'){
			
			$updateObj = new \stdClass;
			$updateObj->value 				= reset($value);
			$updateObj->field_id			= (int)$fieldId;
			$updateObj->item_id				= $itemId;
			$updateObj->context				= $field->context;
			$updateObj->catid				= $catId;
			$updateObj->state				= $state;
			$updateObj->field_name			= $field->title;
			$updateObj->type				= $field->type;
			$updateObj->access				= $field->access;
			$updateObj->ordering			= $field->ordering;
			$updateObj->language			= $lang;
			$updateObj->field_template		= $field_template;
			$updateObj->disabled_categories	= $disabled_categories;
			$updateObj->field_background	= $field_background;

			$this->getDbo()->updateObject('#__jkfilter', $updateObj, array('field_id', 'item_id'));
	...
	public function getFieldValues(array $fieldIds, $itemId)
	{
		...
		if (!array_key_exists($key, $this->valueCache))
		{
			...
			$query->select($query->quoteName(['field_id', 'value']))
				->from($query->quoteName('#__jkfilter'))
