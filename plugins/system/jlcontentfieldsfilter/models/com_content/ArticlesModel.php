...
class ArticlesModel extends ListModel
{
	...
	protected function getListQuery()
	{
		...
		$query->select(
			...
			->join('LEFT', $db->quoteName('#__fields_values', 'x'), $db->quoteName('a.id') . ' = ' . $db->quoteName('x.item_id'))
			->where(
				[
					$db->quoteName('x.field_id') . ' = 15',
				]
			);
		...
		$newString = str_replace('value','cast(value as integer)',$this->getState('list.ordering', 'a.ordering'));

		$query->order(
			$db->escape($newString . ' ' . $db->escape($this->getState('list.direction', 'ASC')))
		);
