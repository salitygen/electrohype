			...
			switch ($fieldsTypes[$k]->type){
				...
				case 'list':
					if(is_array($v) && count($v)){
						...
						foreach ( $v as $val ) {
							if($val !== '')
								$newVal[] = plgSystemJlContentFieldsFilter::strip_data($val);
						}
						$newVal = array_diff($newVal,array(''));
						if(count($newVal)){
							$where = '(field_id = '.(int)$k.' AND value IN(\''.implode("', '",$newVal).'\'))';
						}
					}
					else if(!empty($v)){
						$where = '(field_id = '.(int)$k.' AND value = '.$db->quote(plgSystemJlContentFieldsFilter::strip_data($v)).')';
					}
					break;
				case 'text':
					if(!empty($v)){
						if(is_array($v)){
							...
							$where = '(field_id = '.(int)$k.' AND value LIKE '.$db->quote('%'.plgSystemJlContentFieldsFilter::strip_data($v).'%').')';
						}
					}
					break;
				default:
