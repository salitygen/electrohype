	...
	if(json_last_error() === 0){
		...
		$results = &$db->loadObject();
 		...
 		if($results !== NULL){
			...
			if(!empty($compare = $_SESSION['compare'])){
				if(is_array($compare)){
					if(!in_array($data->id,$compare)){
						array_push($compare,$data->id);
					}
				}
			}else{
				$_SESSION['compare'] = array();
				$compare = array($data->id);
			}

			$compare = array_values(array_unique($compare));

			if(count($compare) > 100){
				$_SESSION['compare'] = array_slice($compare,0,100);
			}else{
